...
const (
	// bitDepth16 is the PCM sample width this muxer assumes. The muxer is
	// 16-bit end to end.
	//
	...
	// It must stay above hls.js's liveSyncDurationCount (three by default),
	// with headroom for a slow poller.
	DefaultWindowSize = 6
...
type Config struct {
	...
	//
	// Callers with a resampling stage upstream should pin this to one value
	// (48000 is the safe choice) rather than follow each source, which removes
	// a whole class of rate-mismatch playback bugs.
	SampleRate int
...
type Stats struct {
	...
	// zero before the first cut. A value that stops advancing means segments
	// have stopped being cut, which is what a liveness monitor should watch.
...
func New(cfg *Config) (*Stream, error) {
	if cfg == nil {
		return nil, errors.New("hls: config is nil")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("hls: codec %q: %w", cfg.Codec.Name, err)
	}
	if enc == nil {
		return nil, fmt.Errorf("hls: codec %q built a nil encoder", cfg.Codec.Name)
	}
	...
	case targetSamples < 1:
		return nil, closeEncoderAndWrap(enc, fmt.Errorf(
			"hls: %s at %d Hz is less than one sample per segment",
			segmentDuration, cfg.SampleRate), cfg.Codec.Name)
	case cfg.Codec.MaxFrameSamples > 0 && targetSamples < cfg.Codec.MaxFrameSamples:
		return nil, closeEncoderAndWrap(enc, fmt.Errorf(
			"hls: %s at %d Hz gives a %d-sample segment, smaller than codec %q's %d-sample access unit",
...
func closeEncoderAndWrap(enc FrameEncoder, err error, codecName string) error {
	...
	return fmt.Errorf("hls: codec %q: %w", codecName, err)
}
...
func (c *Config) validate() error {
	switch {
	case !c.Codec.valid():
		return validationErr("hls: codec is not set")
	case c.SampleRate <= 0:
		return validationErr("hls: sample rate must be positive, got %d", c.SampleRate)
	case c.Channels != 1 && c.Channels != 2:
		return validationErr("hls: channel count must be 1 or 2, got %d", c.Channels)
	case c.BitrateKbps < 0:
		return validationErr("hls: bitrate must not be negative, got %d kbps", c.BitrateKbps)
	case c.SegmentDuration != 0 && c.SegmentDuration < MinSegmentDuration:
		return validationErr("hls: segment duration %s is below the minimum of %s", c.SegmentDuration, MinSegmentDuration)
	case c.SegmentDuration > MaxSegmentDuration:
		return validationErr("hls: segment duration %s exceeds the maximum of %s", c.SegmentDuration, MaxSegmentDuration)
	case c.WindowSize < 0:
		return validationErr("hls: window size must not be negative, got %d", c.WindowSize)
	case c.MaxStallGap < 0:
		return validationErr("hls: max stall gap must not be negative, got %s", c.MaxStallGap)
	}
	return nil
}

// validationErr builds a caller-error with this package's message prefix, so
// the many validate arms stay readable. Errors built here never latch the
// stream; the caller's input was rejected before it changed anything.
func validationErr(format string, args ...any) error {
	return fmt.Errorf(format, args...)
}

// audioErr wraps a runtime encode or container error with the codec context.
func (s *Stream) audioErr(err error) error {
	return fmt.Errorf("hls: codec %q: %w", s.codecName, err)
}
...
func (s *Stream) Write(pcm []byte, ts time.Time) error {
	...
	case s.failed.Load():
		return ErrFailed
	case ts.IsZero():
		return validationErr("hls: write with a zero capture timestamp")
	case len(pcm) == 0:
		return nil
	case len(pcm)%s.frameBytes != 0:
		return validationErr("hls: PCM length %d is not a multiple of the %d-byte sample frame", len(pcm), s.frameBytes)
	}
	...
	if err := s.enc.EncodeInterleaved(pcm, s.emit); err != nil {
		s.failed.Store(true)
		return fmt.Errorf("%w: codec %q: %w", ErrFailed, s.codecName, err)
	}
	return nil
}
...
func (s *Stream) appendAccessUnit(au []byte, samples int) error {
	if samples <= 0 || samples > s.targetSamples {
		return validationErr("hls: encoder emitted an access unit of %d samples, want 1 to %d", samples, s.targetSamples)
	}
...
func (s *Stream) Close() error {
	...
	// code that can panic: a defensive caller may well wrap this call in a
	// recover for exactly that reason. An inline publish would be skipped by
	// the unwind, and since
