...
func newBenchStream(b *testing.B) *hls.Stream {
	b.Helper()
...
func fillWindow(b *testing.B, s *hls.Stream) time.Time {
	b.Helper()

	pcm := tone(testRate, testChannels, testRate, benchToneHz)
	at := testEpoch
	// One second per write and hls.DefaultSegmentDuration per segment, plus a
	// couple of extra seconds so the window is certainly full rather than one
	// cut short of it.
	seconds := hls.DefaultWindowSize*int(hls.DefaultSegmentDuration/time.Second) + 2
	for range seconds {
...
func BenchmarkStreamWriteSegmentCut(b *testing.B) {
	...
	samples := int(hls.DefaultSegmentDuration * testRate / time.Second)
	...
	for b.Loop() {
		...
		at = at.Add(hls.DefaultSegmentDuration)
	}
}
...
// argued on.
func startBenchWriter(b *testing.B, s *hls.Stream, at time.Time) {
	b.Helper()
...
func startBenchPollers(b *testing.B, s *hls.Stream, viewers int) func() float64 {
	b.Helper()
