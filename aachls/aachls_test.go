...
func TestAACLCCodecShape(t *testing.T) {
	...
	assert.NotNil(t, codec.NewEncoder, "AACLC must supply an encoder constructor")
	assert.NotNil(t, codec.WriterConfig, "AACLC must supply a writer configuration")
}

func TestAACLCEncoderReportsASCAndPriming(t *testing.T) {
	t.Parallel()

	enc, err := newAACEncoder(hls.EncoderConfig{
		SampleRate:  testRate,
...
func TestAACLCWriterConfigCarriesPriming(t *testing.T) {
	...
	encCfg := hls.EncoderConfig{SampleRate: testRate, Channels: testChannels, BitrateKbps: testBitrate}
...
func TestAACLCStreamProducesPlayableSegments(t *testing.T) {
	...
	assert.LessOrEqual(t, seg.Duration, hls.DefaultSegmentDuration)

	playlist := s.Playlist()
	assert.Contains(t, playlist, "#EXTM3U")
	assert.Contains(t, playlist, "#EXT-X-MAP:URI=\""+hls.InitSegmentName+"\"")
	assert.Contains(t, playlist, hls.SegmentName(0))
	...
	assert.Equal(t, stats.Retained, strings.Count(playlist, "#EXTINF"),
		"the playlist must advertise exactly the retained segments")
}
