...
func (f *fakeEncoder) DecoderConfig() []byte { return fakeFLACStreamInfo(f.sampleRate, f.channels) }

// fakeFLACStreamInfo builds a populated 34-byte FLAC STREAMINFO. go-m4a's
// fragmented writer rejects an all-zero block (it cannot defer STREAMINFO and
// needs the finalized block up front) and cross-checks the declared sample
// rate against the WriterConfig, so the fake must hand it a real block whose
// rate and channel count agree with the configured stream.
func fakeFLACStreamInfo(sampleRate, channels int) []byte {
	si := make([]byte, flacStreamInfoLen)
	const blockSize = 1024
	binary.BigEndian.PutUint16(si[0:2], blockSize) // min block size
	binary.BigEndian.PutUint16(si[2:4], blockSize) // max block size
	// Packed 8 bytes: sampleRate(20) | channels-1(3) | bitsPerSample-1(5) |
	// totalSamples(36, left zero for an unknown-length live stream).
	const bitsPerSample = 16
	packed := uint64(sampleRate)<<44 | //nolint:gosec // rates in test are within the 20-bit FLAC field
		uint64(channels-1)<<41 | //nolint:gosec // channels is 1 or 2, validated by Config
